import{_ as e,o as r,c as i,a as o}from"./index-flAfBmJv.js";const a={};function s(n,t){return r(),i("div",null,[...t[0]||(t[0]=[o(`<p class="muted" style="margin-bottom:16px;"> RDSS 前端、後端共用的 commit、push 與上板規範。人員與 AI 使用同一份規則，不強制 Pull Request。 </p><div class="callout" style="margin-bottom:24px;background:#f0fdf4;border-color:#86efac;"><p style="color:#166534;margin:0;line-height:1.8;"><strong>核心原則：</strong>在個人分支整合最新 <code>main</code> 並完成驗證，只推送已驗證的 commit SHA；任何拒絕、衝突或異常差異都停止，絕不強制推送。 </p></div><pre style="margin-bottom:28px;">個人分支完成 commit
  → 確認工作目錄乾淨
  → 合併最新 main
  → 驗證與審查差異
  → 固定本次上板 SHA
  → 最後同步閘門
  → 一般 push
  → 確認 SHA 已進入 main</pre><section style="margin-bottom:32px;"><h3 style="font-size:1.3rem;margin-bottom:16px;color:#4f46e5;">第一次使用</h3><div class="grid"><article class="card"><h3>Repository 設定</h3><pre>git config pull.ff only
git config --get pull.ff</pre><p style="margin-top:12px;">預期輸出為 <code>only</code>，避免 Update／Sync 在分支分歧時自動合併。</p></article><article class="card"><h3>遠端 main 保護</h3><ul><li>禁止 force push。</li><li>禁止刪除 <code>main</code>。</li><li>管理者與 bypass 也不能略過。</li><li>保留一般 direct push；不強制 PR。</li></ul></article></div></section><section style="margin-bottom:32px;"><h3 style="font-size:1.3rem;margin-bottom:16px;color:#4f46e5;">請 AI 執行</h3><div class="callout" style="margin-bottom:12px;"><p style="margin:0;color:#3730a3;"> 請先完整讀取開發指南的 <code>15-Git上板流程.md</code>，依規範幫我上板前端及後端，並分別回報驗證與上板結果。 </p></div><p style="color:#64748b;line-height:1.8;"> 只有明確要求「上板」、「併版」或「push」才代表允許遠端 push；僅要求修改、檢查、測試或 commit 時，不代表允許 push。 </p></section><section style="margin-bottom:32px;"><h3 style="font-size:1.3rem;margin-bottom:16px;color:#4f46e5;">標準上板流程</h3><article class="card" style="margin-bottom:18px;"><h3>1. 確認 repository 與分支</h3><pre>git rev-parse --show-toplevel
git branch --show-current
git status --short --branch
git remote -v
git config --get pull.ff</pre><p style="margin-top:12px;">必須位於正確個人分支，不得位於 <code>main</code>；remote 與 <code>pull.ff</code> 也必須正確。</p></article><article class="card" style="margin-bottom:18px;"><h3>2. Commit 後確認乾淨</h3><p>只加入本次任務的明確路徑，不使用批次 <code>git add</code>。</p><pre>git status --porcelain</pre><p style="margin-top:12px;">必須完全沒有輸出，否則驗證內容可能與實際 push 的 commit 不一致。</p></article><article class="card" style="margin-bottom:18px;"><h3>3. 取得並合併最新 main</h3><pre>git status --porcelain
