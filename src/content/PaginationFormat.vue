            ...
            <table>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>Integer</td>
                  <td>✅</td>
                  <td>當前頁碼（<strong>0-based</strong>，Spring Data JPA 原生值，前端 <code style="background: #f3f4f6; padding: 2px 4px; border-radius: 3px;">extractPaginationContent</code> 會自動 +1 轉回 1-based 顯示）</td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td>Integer/Long</td>
                  <td>✅</td>
                  <td>起始索引 = number × size（0-based）</td>
                </tr>
              </tbody>
            </table>
          </div>
                    ...
                    <pre>// API 回應格式
{
  ...
  offset: 0           // 起始索引（選用）
                  ...
                  <div>
                    <p style="margin-bottom: 8px; color: #2563eb;">✓ 格式 2：使用 number/pageSize（Spring Data JPA 原生格式）</p>
                    <pre>// API 回應格式
{
  content: [...],      // 資料陣列
...
return existingPagination.map(entity -&gt; convertToDto(entity));</pre>
            ...
            <div style="margin-top: 16px; padding: 16px; background: #fef3c7; border-left: 3px solid #f59e0b; border-radius: 6px;">
              <p style="color: #92400e; font-weight: 600; margin-bottom: 8px;">⚠️ 注意：</p>
              <ol style="color: #92400e; margin: 0; padding-left: 20px; line-height: 1.8;">
                <li>Spring Data JPA 的頁碼從 0 開始，前端傳入的 pageNo 從 1 開始。可使用 <code style="background: #fde68a; padding: 2px 6px; border-radius: 4px;">RequestUtils.getPageRequest(req)</code> 自動轉換，或自行處理 -1 的計算。</li>
                <li><code style="background: #fde68a; padding: 2px 6px; border-radius: 4px;">Pagination&lt;T&gt;</code> 的 <code style="background: #fde68a; padding: 2px 6px; border-radius: 4px;">number</code> 欄位保持 Spring Data JPA 原生的 <strong>0-based</strong> 值，前端 <code style="background: #fde68a; padding: 2px 6px; border-radius: 4px;">extractPaginationContent</code> 會自動 +1 轉為顯示用的 1-based 頁碼，後端<strong>不需要</strong>自行做 +1 轉換。</li>
              </ol>
            </div>
          </div>
