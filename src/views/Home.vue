  ...
  <div class="min-w-0">
    ...
    <header class="hero sticky top-0 z-50 bg-gradient-to-b from-indigo-50 to-slate-50 border-b border-gray-200 px-3 py-2 sm:px-4 sm:py-3 md:px-6 lg:px-8">
      <nav class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-3 mb-2 sm:mb-3">
        <div class="font-bold text-sm sm:text-base lg:text-lg tracking-wide break-words max-w-full">RDSS Development Guide</div>
      </nav>
          ...
          >
            <template #header>
              <div class="flex items-center justify-between w-full">
                ...
                <span
                  v-if="section.protected && !isProtectedUnlocked"
                  style="font-size:1rem;margin-right:8px;opacity:0.55;"
                  title="需要密碼"
                >🔒</span>
              </div>
            </template>
    ...
    <footer class="py-8 md:py-10 px-4 text-center text-gray-500 border-t border-gray-200">
      <p class="text-sm md:text-base">© 2026 RDSS Development Guide</p>
    </footer>
  </div>
</template>
...
import PasswordGate from '@/content/PasswordGate.vue'

const isDownloading = ref(false)

// ── Protected section auth ─────────────────────────────────────────────
const _pk = '_p'
const isProtectedUnlocked = ref(false)

function handleUnlock(token) {
  isProtectedUnlocked.value = true
  localStorage.setItem(_pk, token)
}

// Named reference so it can be properly removed from beforeunload
const _clearAuth = () => localStorage.removeItem(_pk)
// ───────────────────────────────────────────────────────────────────────
...
const sections = [
  ...
  { id: 'section-10', title: '10. Graph API 寄信', component: defineAsyncComponent(() => import('@/content/GraphMailSpec.vue')) },
  ...
  { id: 'section-15', title: '15. Git 上板流程', component: defineAsyncComponent(() => import('@/content/GitWorkflow.vue')) },
  // ⚠️ 規則：測試環境與測試帳號永遠排在最後兩項，新增規範請插在它們上方。詳見 _dev-notes/_SECTION_ORDER_RULE.md
  { id: 'appendix', title: '16. 測試環境', component: defineAsyncComponent(() => import('@/content/TestEnvironment.vue')), protected: true },
  { id: 'section-last', title: '17. 測試帳號', component: defineAsyncComponent(() => import('@/content/TestAccounts.vue')), protected: true }
]
...
onMounted(() => {
  ...
  // Restore auth if still within the same browser session (not yet refreshed)
  const _stored = localStorage.getItem(_pk)
  if (_stored && /^[0-9a-f]{16}$/.test(_stored)) {
    isProtectedUnlocked.value = true
  }
  // Clear auth on page unload — refresh 重置保護狀態
  window.addEventListener('beforeunload', _clearAuth)
})
...
onBeforeUnmount(() => {
  ...
  window.removeEventListener('beforeunload', _clearAuth)
})
</script>
